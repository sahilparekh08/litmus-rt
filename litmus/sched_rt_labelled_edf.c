...
static void demo_requeue(struct task_struct *tsk, struct demo_cpu_state *cpu_state)
{
        if(tsk->rt_validity == INVALID) {
                TRACE_TASK(tsk, "Unable to requeue task since it is not RT valid\n");
                return;
        }
...
static struct task_struct* demo_schedule(struct task_struct * prev)
{
	...
	TRACE_TASK(prev, "rt_validity of prev is %d\n", prev->rt_validity);
         ...
         * scheduling decision.
         */
        TRACE_TASK(prev, "enter demo_schedule_end without goto\n");
demo_schedule_end:
        sched_state_task_picked();
        TRACE_TASK(prev, "scheduling decision made\n");
                          ...
                          int is_running)
{
        ...
        if(tsk->rt_validity == INVALID) {
                TRACE_TASK(tsk, "Unable to release task since it is not RT valid\n");
                return;
        }
        ...
        TRACE_TASK(tsk, "end of demo_task_new()\n");
}
...
static void demo_task_exit(struct task_struct *tsk)
{
        ...
        TRACE_TASK(tsk, "inside demo_task_exit()\n");
...
static void demo_task_resume(struct task_struct  *tsk)
{
        ...
        /* Check this before acquiring the lock */
        if(tsk->rt_validity == INVALID) {
                TRACE_TASK(tsk, "Unable to RESUME task since it is not RT valid\n");
                return;
        }
